...
def wish_list(wish_list_items, selected_item, item_cost, budget):
    if selected_item in wish_list_items:
        if item_cost <= budget:
            return f"You should buy a {selected_item}!"
        else:
            return f"You should save up for a {selected_item}!"
    else:
        return f"You probably don't want to buy a {selected_item}."
