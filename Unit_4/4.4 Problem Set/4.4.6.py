...
def write_weird_file(filename, input_list, mode="w", sort_first=False, reverse_first=False):
    # Sorting the list if sort_first is True
    if sort_first:
        input_list.sort()
    
    # Reversing the list if reverse_first is True
    if reverse_first:
        input_list.reverse()
    
    # Opening the file based on the mode
    file = open(filename, mode)
    
    # Writing each item from the list to the file
