...
def word_lengths(input_string):
    ...
    words = input_string.replace('.', '').replace(',', '').replace('!', '').replace('?', '').replace("'", '').split()
    
    # Convert each word to lowercase and create dictionary
    word_length_dict = {}
    
    for word in words:
        lowercase_word = word.lower()
        if lowercase_word not in word_length_dict:
            word_length_dict[lowercase_word] = len(lowercase_word)
