...
def alphabetical_keys(dictionary):
    # Get the keys (student names) from the dictionary and sort them alphabetically
    sorted_names = sorted(dictionary.keys())
    
    # Initialize an empty list to collect the formatted strings
    result = []
    
    # Iterate through sorted names and construct the output format
    for name in sorted_names:
        result.append(f"{name}: {dictionary[name]}")
