...
def most_active(dictionary):
    ...
    max_value = -1  # Start with a value lower than any possible days of activity
    max_key = None
    
    # Iterate through the dictionary to find the maximum value and corresponding key
    for key, value in dictionary.items():
        if value > max_value:
            max_value = value
            max_key = key
    
    # Return the key with the maximum value (most active student)
    return max_key
