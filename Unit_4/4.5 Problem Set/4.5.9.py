...
def clean_data(input_dict):
    ...
    for key, value in input_dict.items():
        # Check if value is an integer or float and within valid range
        if not (isinstance(value, int) or isinstance(value, float)) or value < 0 or value > 100:
            keys_to_delete.append(key)
    
    # Delete invalid key-value pairs
