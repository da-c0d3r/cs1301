...
def stars(movies, tvshows):
    ...
    # Process movies dictionary
    for movie, performers in movies.items():
        for performer in performers:
            if performer in performer_dict:
                performer_dict[performer].append(movie)
            else:
                performer_dict[performer] = [movie]
    
    # Process tvshows dictionary
    for tvshow, performers in tvshows.items():
        for performer in performers:
            if performer in performer_dict:
                performer_dict[performer].append(tvshow)
            else:
                performer_dict[performer] = [tvshow]
    
    # Sort the lists of shows and movies alphabetically
    for performer, shows_movies in performer_dict.items():
        shows_movies.sort()
