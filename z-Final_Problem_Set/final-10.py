...
def preprocess_message(message):
    ...
    pairs = [(pair[0], 'X') if pair[0] == pair[1] else pair for pair in pairs]
    return pairs

def apply_cipher(pairs, encrypt=True):
    result = []
    for char1, char2 in pairs:
        ...
        if row1 == row2:
            # Same row
            new_col1 = (col1 + (1 if encrypt else -1)) % 5
            new_col2 = (col2 + (1 if encrypt else -1)) % 5
            result.append(CIPHER[row1][new_col1])
            result.append(CIPHER[row2][new_col2])
        elif col1 == col2:
            # Same column
            new_row1 = (row1 + (1 if encrypt else -1)) % 5
            new_row2 = (row2 + (1 if encrypt else -1)) % 5
            result.append(CIPHER[new_row1][col1])
            result.append(CIPHER[new_row2][col2])
        else:
            # Rectangle
            result.append(CIPHER[row1][col2])
            result.append(CIPHER[row2][col1])
    
    return ''.join(result)

def encrypt(message):
    pairs = preprocess_message(message)
    return apply_cipher(pairs, encrypt=True)

def decrypt(message):
    pairs = [message[i:i+2] for i in range(0, len(message), 2)]
    return apply_cipher(pairs, encrypt=False)
